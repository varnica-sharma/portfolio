// FLOATING TAGS HERO - with fun icons
const tags = [
  { text: "Machine Learning", icon: "bx-brain" },
  { text: "Time Series Forecasting", icon: "bx-line-chart" },
  { text: "Graph Neural Networks", icon: "bx-network-chart" },
  { text: "Deep Learning", icon: "bx-chip" },
  { text: "XGBoost & CatBoost", icon: "bx-trending-up" },
  { text: "PyTorch", icon: "bx-code-curly" },
  { text: "Healthcare AI", icon: "bx-health" },
  { text: "NLP & Transformers", icon: "bx-message-dots" },
  { text: "AWS Cloud", icon: "bx-cloud" },
  { text: "Predictive Modeling", icon: "bx-bar-chart-alt-2" },
  { text: "Data Pipelines", icon: "bx-data" },
  { text: "Streamlit", icon: "bx-window" }
];

const heroTagPositions = [
  [20, 5], [70, 10], [120, 5], [170, 15],
  [220, 8], [270, 20], [320, 10], [370, 18],
  [50, 40], [100, 45], [150, 38], [200, 48]
];
...
if (tagsContainer) {
  tags.forEach((tag, idx) => {
    if (idx < heroTagPositions.length) {
      ...
      tagEl.className = 'float-tag';
      tagEl.style.top = heroTagPositions[idx][0] + 'px';
      tagEl.style.left = heroTagPositions[idx][1] + '%';
      tagEl.style.animationDelay = (idx * 0.3) + 's';
      tagEl.innerHTML = `<i class='bx ${tag.icon}'></i> ${tag.text}`;
